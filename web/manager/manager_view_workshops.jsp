...
<%@ include file="/masthead.jsp" %>
		<div id="headerContainer">
		       <div id="header"><h1>View Workshops</h1></div>
		...
		<div id="mainContent">
			...
			<input type="button" value="Update" onclick="view_workshop_view();"/>
			<hr />
			<s:if test="workshops.size() > 0">
				<s:iterator value="workshops">
					<s:property value="subject" /><br />
					<ul>
						<s:set name="sessions" value="#workshops.sessions" />
						<s:if test="sessions.size() > 0">
					 		<s:iterator value="sessions">
								<s:url id="viewURL" action="manage_session_view.do">
									<s:param name="id" value="id" />			
								</s:url>
					 			<li><s:a href="%{viewURL}"><s:property value="name"/></s:a> 
					 				(<s:property value="workshopReservations.size()" />/<s:property value="maxCapacity"/>)</li>
				 			</s:iterator>
						</s:if>
						<s:else>
							<li>No sessions</li>
