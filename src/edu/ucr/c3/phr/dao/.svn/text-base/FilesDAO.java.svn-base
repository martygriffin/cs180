 ...
 * 
 * @see edu.ucr.c3.wms.dao.Files
 * @author MyEclipse Persistence Tools
 */

public class FilesDAO extends HibernateDaoSupport {
	...
	public Files findById(java.lang.Integer id) {
		...
		try {
			Files instance = (Files) getHibernateTemplate().get(
					"edu.ucr.c3.phr.domain.Files", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public Files merge(Files detachedInstance) {
		...
		try {
			Files result = (Files) getHibernateTemplate().merge(
					detachedInstance);
	...
	public static FilesDAO getFromApplicationContext(ApplicationContext ctx) {
		return (FilesDAO) ctx.getBean("FilesDAO");
	}
}